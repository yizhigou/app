...
class wp_thread_comment{
	...
	function defaultoption($key=''){
		...
		if($key === 'cancel_reply'){
			...
			return __('<div class="comment-childs<?php echo $deep%2 ? \' chalt\' : \'\'; ?>" id="comment-[ID]"><?php if(function_exists("get_avatar")) echo get_avatar( $comment, 32 ); ?><cite>[author]</cite> Reply222:[moderation]<br /><small class="commentmetadata">[date] at [time]</small>[content]</div>','wp-thread-comment');
...
/*	function activate(){
	...
	function commenttext($deep=0,$count=0){
		...
		if(function_exists('wpthreadcomment_threadedcomment')){
			...
			$p = str_replace('[date]', get_comment_date('l, F jS, Y'), $p);
