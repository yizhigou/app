...
//                        if($_GET["where$i"] == $k) { print " SELECTED "; }
        ...
        # query builder
            ...
            $fields="";
            for($i=1;$i<=5;$i++) {
                if($_GET["where$i"] != '' && array_key_exists($_GET["where$i"], $f)) {//校验where变量确保合法
                    $fields.=$_GET["where$i"].",";
                }
            }
            $fields=rtrim($fields,",");
            ...
            for($i=1;$i<=5;$i++) {
                if(($_GET["what$i"] != '') && ($_GET["where$i"] != '')) {
                    $where.=" AND ".$_GET["where$i"]." LIKE '%".addslashes($_GET["what$i"])."%'";//addslashes防止注入攻击
                }
            }
            # ORDER BY
            $orderby="";
